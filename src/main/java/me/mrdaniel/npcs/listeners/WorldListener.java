...
public class WorldListener {
	...
	public void onWorldLoad(final LoadWorldEvent e) {
		...
		world.getEntities().stream().filter(ent -> ent.get(NPCData.class).isPresent() || (ent instanceof NPCAble && ((NPCAble)ent).getNPCFile() != null)).forEach(npc -> npc.remove());
		files.forEach(file -> {
			try { NPCManager.getInstance().spawn(file); }
			catch (final NPCException exc) { NPCs.getInstance().getLogger().error("Failed to spawn NPC: " + exc.getMessage(), exc); }
		});
	}
}