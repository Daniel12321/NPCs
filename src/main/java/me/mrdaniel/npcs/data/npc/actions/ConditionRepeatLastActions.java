...
public class ConditionRepeatLastActions extends ConditionActions {

	public ConditionRepeatLastActions(@Nonnull final List<Action> actions, @Nonnull final List<Action> completeactions, @Nonnull final Map<UUID, Integer> current, @Nonnull final List<Condition> conditions, @Nonnull final List<UUID> completed) {
		super("condition_repeat_last", actions, completeactions, current, conditions, completed);
	}
	...
	public void execute(final NPCs npcs, final Player p, final Living npc) {
		...
		if (current < super.current.size()) {
			...
			else actions.get(actions.size()-1).execute(npcs, p, npc);
		}
	}
}